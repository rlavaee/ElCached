...
typedef struct {
    ...
    unsigned int list_size; /* size of prev array */
		...
		void **sorted_list;
		hyperLogLogState *hlls;
		unsigned int mon_slabs;
...
static pthread_mutex_t mon_slabs_lock = PTHREAD_MUTEX_INITIALIZER;
...
static void slabs_output(void);
void start_slab_monitor(void);
void stop_slab_monitor(void);
static int ptr_comp(const void * p1, const void * p2);
static size_t my_lower_bound(const void * value, const void ** base, size_t num, size_t size);
int slabs_deallocate(const size_t new_mem_limit);

/*
static void *slab_deallocate_thread(void *arg) {
		sleep(60);
		start_slab_monitor();
		sleep(10);
		fprintf(stderr,"under provisioning to 128\n");
		slabs_deallocate(64 << 20);
		stop_slab_monitor();
    slabs_output();
...
void slabs_init(const size_t limit, const double factor, const bool prealloc) {
    ...
    while (++i < MAX_NUMBER_OF_SLAB_CLASSES-1 && size <= settings.item_size_max / factor) {
        ...
        if (settings.verbose > 1) {
	     ...
	     /* rlavaee: set chunk size for slab class */
	     rd_trace_set_chunk_size(i,size);
    ...
    if (settings.verbose > 1) {
		...
		/* rlavaee set chunk size for the largest item size */
    rd_trace_set_chunk_size(i,settings.item_size_max);
...
static int do_slabs_newslab(const unsigned int id) {
    ...
    if ((mem_limit && mem_malloced + len > mem_limit && p->slabs > 0
         && g->slabs == 0)) {
		    ...
		    //fprintf(stderr,"mem_limit: %zu, mem_malloced: %zu len: %d\n",mem_limit,mem_malloced,len);
    ...
    if (settings.verbose > 1) 
		  fprintf(stderr,"increasing p->slabs\n");
...
void *slabs_alloc(size_t size, unsigned int id, unsigned int *total_chunks,
    ...
    void *ret;
		...
		pthread_mutex_lock(&mon_slabs_lock);
    pthread_mutex_lock(&slabs_lock);
    ret = do_slabs_alloc(size, id, total_chunks, flags);
    pthread_mutex_unlock(&slabs_lock);
		pthread_mutex_unlock(&mon_slabs_lock);
    return ret;
}
...
static int slab_rebalance_start(void) {
     ...
     */
		if(is_monitoring_slabs()){
			assert(s_cls->mon_slabs >= 1 && "ERROR few slabs");
		  unsigned min_index = 0;
		  double min_estimate = estimateHyperLogLog(&s_cls->hlls[0]);
		  for(unsigned i=1; i < s_cls->mon_slabs; ++i){
		  	double estimate;
			  if((estimate = estimateHyperLogLog(&s_cls->hlls[i])) < min_estimate){
        ...
        if (settings.verbose > 1) 
			...
			assert(min_index < s_cls->slabs && "ERROR out of bound slabs");
		  slab_rebal.slab_start = s_cls->slab_list[min_index];
			slab_rebal.slab_index = min_index;
		  //fprintf(stderr,"min_estimate: %f\n min_index: %u\n",min_estimate,min_index);
		}else{
			assert(false && "ERROR should not be here");
      slab_rebal.slab_start = s_cls->slab_list[0];
			slab_rebal.slab_index = 0;
		}
...
static int slab_rebalance_remove(void){

    pthread_mutex_lock(&slabs_lock);

	  slabclass_t *s_cls = &slabclass[slab_rebal.s_clsid];
    int x;
    int was_busy = 0;
    int refcount = 0;
    uint32_t hv;
                ...
                if ((hold_lock = item_trylock(hv)) == NULL) {
                    ...
                    if (refcount == 2) { /* item is linked but not busy */
                        ...
                        if (settings.verbose > 2) {
                            fprintf(stderr, "Slab remove hit a busy item: refcount: %d (%d)\n",
...
static void slab_remove_finish(void) {
    slabclass_t *s_cls;
    int x;
    //uint32_t rescues;
    ...
    s_cls->slabs--;
		...
		if(is_monitoring_slabs()){
			s_cls->mon_slabs--;
		  for (x = slab_rebal.slab_index; x < s_cls->mon_slabs; x++){
			  s_cls->hlls[x] = s_cls->hlls[x+1];
				s_cls->sorted_list[x] = s_cls->sorted_list[x+1];
			}
		}

    for (x = slab_rebal.slab_index; x < s_cls->slabs; x++) {
        s_cls->slab_list[x] = s_cls->slab_list[x+1];
    }

    //d_cls->slab_list[d_cls->slabs++] = slab_rebal.slab_start;
		//rlavaee: free the slab
		free(slab_rebal.slab_start);
    int len = settings.slab_reassign ? settings.item_size_max
        : s_cls->size * s_cls->perslab;
		mem_malloced -= len;
		//fprintf(stderr,"mem_malloced is now: %zu\n",mem_malloced);
    ...
    pthread_mutex_unlock(&mon_slabs_lock);
	  ...
	  //fprintf(stderr,"slab_remove finished\n");

    if (settings.verbose > 1) {
        fprintf(stderr, "finished a slab remove\n");
    }
}
...
static void *slab_rebalance_thread(void *arg) {
    ...
    while (do_run_slab_rebalance_thread) {
        if (slab_rebalance_signal == 1) {
					 //fprintf(stderr,"signal is 1\tstarting rebalance\n");
            ...
            was_busy = 0;
					 ...
					 //fprintf(stderr,"signal is 2\tdoing rebalance\n");
					  if(slab_rebal.d_clsid == -1)
							was_busy = slab_rebalance_remove();
						else
              was_busy = slab_rebalance_move();
        } 

        if (slab_rebal.done) {
					  //fprintf(stderr,"slab rebalance done\n");
					  if(slab_rebal.d_clsid == -1)
							slab_remove_finish();
						else
              slab_rebalance_finish();
        } else if (was_busy) {
					  //fprintf(stderr,"slab rebalance busy\n");
            /* Stuck waiting for some items to unlock, so slow down a bit
             * to give them a chance to free up */
            usleep(50);
        }

        while (slab_rebalance_signal == 0) {
					  //fprintf(stderr,"waiting to rebalance\n");
...
//static pthread_t deallocate_tid;
...
int start_slab_maintenance_thread(void) {
    ...
    if ((ret = pthread_create(&rebalance_tid, NULL,
		...
		/*
		if(( ret = pthread_create(&deallocate_tid, NULL,
				                      slab_deallocate_thread, NULL)) != 0){
		    fprintf(stderr, "Can't create dealloc thread: %s\n",strerror(ret));
				return -1;
		}
		*/
    return 0;
...
static int slabs_remove_pick_rand(void);

int slabs_deallocate (const size_t new_mem_limit){

	 fprintf(stderr,"starting slab deallocate\n");
	 int src;
   ...
   mutex_lock(&slabs_rebalance_lock);
    ...
    if (settings.verbose > 1) 
		 fprintf(stderr,"attempting new slab_remove: %zu %zu\n",mem_malloced,mem_limit);


	   do{
       src = slabs_remove_pick_rand();
       if (settings.verbose > 1) 
			   fprintf(stderr,"source was: %u\n",src);
	   }while(src < POWER_SMALLEST || src > power_largest || slabclass[src].slabs < 2);

     slab_rebal.s_clsid = src;
		 slab_rebal.d_clsid = -1;

     slab_rebalance_signal = 1;
     pthread_cond_signal(&slab_rebalance_cond);
		 while(slab_rebalance_signal != 0){
			 //fprintf(stderr,"waiting for rebalance finish\n");
			 /* slab reassign running */
       pthread_cond_wait(&slab_rebalance_cond, &slabs_rebalance_lock);
		 }
		 if(mem_malloced <= mem_limit){
			 mutex_unlock(&slabs_rebalance_lock);
			 break;
		 } 
	}

	//fprintf(stderr,"finally: mem_malloced: %zu, mem_limit: %zu\n",mem_malloced,mem_limit);
...
static int slabs_remove_pick_rand(void) {
	  srand(time(NULL));
		...
		slabs_output();
	  int total_slabs = 0;
    for(int cur = POWER_SMALLEST - 1; cur < power_largest; ++cur){
			 //fprintf(stderr,"slabclass: %d\t%d\n",cur,slabclass[cur].slabs);
				if(slabclass[cur].slabs > 1) 
					total_slabs += slabclass[cur].slabs;
    }
		//fprintf(stderr,"total slabs: %d\n",total_slabs);

		int r = rand()%total_slabs;
...
static void slabs_output(void){
	//fprintf(stderr,"***************************\n");
    for(int cur = POWER_SMALLEST - 1; cur < power_largest; ++cur){
					//fprintf(stderr,"slab class: %d: %d\n",cur,slabclass[cur].slabs);
    }
}

static size_t my_lower_bound(const void * value, 
		                           const void ** first, 
															    size_t num, 
																	  size_t size){
	size_t index, base = 0;
	size_t step, count = num;

	while (count > 0) {
		step = count / 2;
		index = base + step;
		//fprintf(stderr,"base: %zu, index: %zu, %p < %p: %d\n",base,index,first[index],value,(uint64_t)first[index] < (uint64_t)value);
		if((uint64_t)first[index] < (uint64_t)value){
		//if (memcmp(first+index,&value,size) < 0){
			base = index;
			count -= step + 1; 
		}
		else if((uint64_t)first[index] == (uint64_t)value)
			return index;
		else
			count = step;
	}
...
void start_slab_monitor(void){
	fprintf(stderr,"starting slab monitoring\n");

	pthread_mutex_lock(&mon_slabs_lock);
	pthread_mutex_lock(&slabs_lock);

  for(int cur = POWER_SMALLEST - 1; cur < power_largest; ++cur){
		slabclass[cur].mon_slabs = slabclass[cur].slabs;
		//fprintf(stderr,"cur: %u\t mon_slabs:%u\n",cur,slabclass[cur].mon_slabs);
		if(slabclass[cur].mon_slabs != 0){
		  slabclass[cur].sorted_list = malloc(sizeof(void*)*slabclass[cur].mon_slabs);
		  memcpy(slabclass[cur].sorted_list, slabclass[cur].slab_list, sizeof(void*)*slabclass[cur].mon_slabs);
		  slabclass[cur].hlls = malloc(slabclass[cur].mon_slabs * sizeof(hyperLogLogState));
		  for(int i=0; i < slabclass[cur].mon_slabs ; ++i)
			  initHyperLogLog(&slabclass[cur].hlls[i],16);
		}
		qsort(slabclass[cur].sorted_list, slabclass[cur].mon_slabs, sizeof(void*), ptr_comp);
		//for(int i=0; i<slabclass[cur].mon_slabs; ++i)
			//fprintf(stderr,"BIBI: %u: sorted list[%d]: %p\n",cur,i,slabclass[cur].sorted_list[i]);
  }
	pthread_mutex_unlock(&slabs_lock);
	set_monitoring_slabs(true);
	pthread_mutex_unlock(&mon_slabs_lock);


	//fprintf(stderr,"really starting slab monitoring\n");
}


void stop_slab_monitor(void){

	fprintf(stderr,"stoping slab monitoring\n");

	pthread_mutex_lock(&mon_slabs_lock);
	set_monitoring_slabs(false);
	pthread_mutex_lock(&slabs_lock);

  for(int cur = POWER_SMALLEST - 1; cur < power_largest; ++cur){
		  for(int i=0; i < slabclass[cur].mon_slabs ; ++i){
			  //fprintf(stderr,"class: %u\t i: %d, list_size: %u\n",cur,i,slabclass[cur].mon_slabs);
		    free(slabclass[cur].hlls[i].hashesArr);
		  }
		 if(slabclass[cur].mon_slabs!=0){
			 //fprintf(stderr,"freeing hlls %p\n",(void*)slabclass[cur].hlls);
	     free(slabclass[cur].hlls);
			 //fprintf(stderr,"freeing sorted list %p\n",(void*)slabclass[cur].sorted_list);
			 free(slabclass[cur].sorted_list);
		 }
  }
...
void set_slab_active(item * it, const uint32_t hv){

	pthread_mutex_lock(&mon_slabs_lock);
	if(is_monitoring_slabs()){


	  unsigned cur = ITEM_clsid(it);
	  assert(slabclass[cur].mon_slabs == slabclass[cur].slabs && "NOT CONSISTENT");
	  //fprintf(stderr,"setting slab active\t %u\t%u\t%p\t%u\n",cur,hv,(void*)it,slabclass[cur].mon_slabs);

	  size_t index = my_lower_bound((const void *)it, (const void**)slabclass[cur].sorted_list, slabclass[cur].mon_slabs, sizeof(void*));

	//fprintf(stderr,"index is: %zu\t out of %u, cur:%u, it: %p, index ptr: %p\n",index,slabclass[cur].mon_slabs,cur,(void*)it,slabclass[cur].sorted_list[index]);
	  assert (index < slabclass[cur].mon_slabs && "OUT OF BOUND index");
	
		uint64_t it_int = (uint64_t)it;
		_unused(it_int);
	  uint64_t index_int = (uint64_t)slabclass[cur].sorted_list[index];
		_unused(index_int);

		assert((it_int >= index_int) && ("ERROR"));
	  assert((it_int <= index_int + slabclass[cur].perslab * slabclass[cur].size) && "ERROR");
    if (settings.verbose > 1) 
		  fprintf(stderr,"adding hyperloglog: cur: %u, index: %zu\n",cur,index);
	  addHyperLogLog(&slabclass[cur].hlls[index],hv);
  }
	pthread_mutex_unlock(&mon_slabs_lock);
}

/* rlavaee get total allocated chunks for a slab class */
unsigned int get_chunk_count(unsigned int slab_clsid){
  slabclass_t *p = &slabclass[slab_clsid];
  return p->slabs * p->perslab - p->sl_curr;
}

unsigned int get_slab_count(unsigned int slab_clsid){
  slabclass_t *p = &slabclass[slab_clsid];
  return p->slabs;
}

size_t get_mem_malloced(){
	return mem_malloced;
}
