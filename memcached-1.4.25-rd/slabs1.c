...
static void start_slab_monitor(void);
static void stop_slab_monitor(void);
...
void set_slab_active(item * it, const uint32_t hv){
	...
	if(index < slabclass[cur].mon_slabs){
		uint64_t it_int = (uint64_t)it;
	  uint64_t index_int = (uint64_t)slabclass[cur].sorted_list[index];
		if((it_int >= index_int) && (it_int <= index_int + slabclass[cur].perslab * slabclass[cur].size))
