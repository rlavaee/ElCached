...
zipf <- read.table(file.path("..","memcached-data","mrc-4Bkey-50Mreq-1.15alpha.csv"),sep="\t",col.names=c("size","mr"))
exp <- read.table(file.path("..","memcached-data","mrc-4Bkey-50Mreq-exp0.00001.csv"),sep="\t",col.names=c("size","mr"))
zipf[,"workload"] <- c("zipf-1.15-4Bkey-50Mreq")
exp[,"workload"] <- c("exp-0.00001-4Bkey-50Mreq")

zipf.approx = approx(zipf$size, zipf$mr, n=100)
exp.approx = approx(exp$size, exp$mr, n=100)
zipf = data.frame(size=zipf.approx$x, mr=zipf.approx$y)
exp = data.frame(size=exp.approx$x, mr=exp.approx$y)
df <- rbind(zipf,exp)


df.join <- merge(zipf,exp,by=NULL)
