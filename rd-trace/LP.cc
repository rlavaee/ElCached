...
int gen_solve_lp (std::vector< std::vector <std::pair<unsigned,float> > > &mrc, unsigned &L1_size, unsigned &L2_size){
  std::vector<float> min_lats;
	...
	if(ret == 0){
	  set_add_rowmode(lp, TRUE); /*makes building the model faster if it is done rows by row*/
		...
		for(unsigned i=0; i < mrc.size(); ++i){
		  for(unsigned j=0 ; j < mrc[i].size() -1 ; ++j ){
				if(ret == 0){
				  auto p1 = mrc[i][j];
				  auto p2 = mrc[i][j+1];
	...
	//	int k = 0;
	//	for(unsigned i=0; i<total_memcached_count; ++i){
	//	  colno[k] = 6*i+1;
	//		row[k++] = 1;
	//	}
