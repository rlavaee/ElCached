...
int ProtocolBinary::get_request(const char* key) {
  ...
  // rlavaee GET --> GETK
  binary_header_t h = { 0x80, CMD_GETK, htons(keylen),
                        0x00, 0x00, {htons(0)},
                        htonl(keylen) };
...
bool ProtocolBinary::handle_response(evbuffer *input, bool &done){
  //fprintf(stderr,"handling response original way\n");
  ...
  if ((h->opcode == CMD_GET || h->opcode == CMD_GETK) && h->status) {
  ...
  //fprintf(stderr,"read %x, %x bytes\n",ntohl(h->body_len),targetLen);
  done = true;
  return true;
}
 ...
 * @return  true if consumed, false if not enough data in buffer.
 */
bool ProtocolBinary::handle_response(evbuffer *input, bool &done, char * keystr, bool &missed){
  missed =false;
  //fprintf(stderr,"handling response\n");
  // Read the first 24 bytes as a header
  int length = evbuffer_get_length(input);
  if (length < 24) return false;
  binary_header_t* h =
  ...
  // If something other than success, count it as a miss
  //rlavaee GET->GETK
  if (h->opcode == CMD_GETK && h->status) {
      missed = true;
      conn->stats.get_misses++;
      //fprintf(stderr,"miss occured\n");
      uint16_t key_len = ntohs(h->key_len);
      //unsigned char * key = (unsigned char *)malloc(key_len*sizeof(unsigned char));
      //fprintf(stderr,"attempting to read %d bytes from buffer.\n",key_len); 
      unsigned char * key = evbuffer_pullup(input,key_len);
      //fprintf(stderr,"key len is : %d\n",key_len);
      strncpy(keystr, (const char *) key, key_len);
      keystr[key_len]='\0';
      //fprintf(stderr,"key: %s was missed\n",keystr);
      //fprintf(stderr,"returning info\n");
  }
  ...
  //evbuffer_drain(input, targetLen);
  //rlavaee
  evbuffer_drain(input,ntohl(h->body_len));
  conn->stats.rx_bytes += targetLen;
  //fprintf(stderr,"read %x, %x bytes\n",ntohl(h->body_len),targetLen);
  done = true;
  return true;
