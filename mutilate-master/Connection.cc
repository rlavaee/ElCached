...
{
	total_records = ((uint64_t)(options.records))*1000000;
  valuesize = createGenerator(options.valuesize);
  keysize = createGenerator(options.keysize);
  keydist = createGenerator(options.keydist) ;
	std::cerr << "total record: " << total_records;
  keygen = new KeyGenerator(keysize, total_records);
...
void Connection::start_loading() {
  ...
  for (int i = 0; i < LOADER_CHUNK; i++) {
    if (loader_issued >= total_records) break;
    char key[256];
    int index = lrand48() % (1024 * 1024);
    string keystr;
    if(options.distset){
			/* Get the key distribution from the given distribution*/
    	keystr = keygen->generate(keydist->generate());
		} else {
			/* Get a completely random key */
...
void Connection::issue_something(double now) {
  char key[256];
  string keystr = keygen->generate(keydist->generate());
  strcpy(key, keystr.c_str());
...
bool Connection::check_exit_condition(double now) {
	if (total_issued >= options.nrequests) return true;
...
void Connection::drive_write_machine(double now) {
  ...
  while (1) {
    ...
    case ISSUING:
			...
			if (total_issued >= options.nrequests) return;
      ...
      if (options.skip && options.lambda > 0.0 &&
        ...
        while (next_time < now - 0.004000) {
          ...
          next_time += iagen->generate();
			...
			//std::cerr << std::fixed << "next_time: " << next_time << ", now: "<< now << "\t" << op_queue.size() << "\n";
      break;
...
void Connection::read_callback() {
  ...
  while (1) {
    ...
    case WAITING_FOR_GET:
      ...
      //rlavaee: reload the cache with the missed item
    	char  _key[256];
	bool missed;
      full_read = prot->handle_response(input, done, _key, missed);
      if (!full_read) {
        return;
      } else if (done) {
        finish_op(op); // sets read_state = IDLE
	if(missed && options.reload){
	 
      	// rlavaee: issueing set
	 write_state = ISSUING;
          int index = lrand48() % (1024 * 1024);
	  //fprintf(stderr,"issuing set for: %s\n",_key);
         issue_set(_key, &random_char[index], valuesize->generate());
	 }
      }
      break;
    ...
    case LOADING:
      ...
      if (loader_completed == total_records) {
        D("Finished loading.");
        read_state = IDLE;
      } else {
